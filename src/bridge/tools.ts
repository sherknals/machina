...
function loadConfig(): BridgeConfig {
  const configPaths = [
    join(process.cwd(), 'machina.config.json'),
  ...
  for (const configPath of configPaths) {
    if (existsSync(configPath)) {
      ...
      } catch { /* Invalid config, use default */ }
    }
  }
...
export const coreTools: SystemTool[] = [
  ...
  },
  {
    ...
    handler: async (args) => {
      ...
      const { command, args: cmdArgs, error, useShell: _useShell } = validateShellCommand(fullCommand);
...
export const calendarTools: SystemTool[] = [
  ...
  },
  {
    ...
    handler: async (args) => {
      const title = String(args.title || '').replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      const dateInput = String(args.date || '').replace(/[\r\n]+/g, ' ').trim();
      const timeInput = String(args.time || '').replace(/[\r\n]+/g, ' ').trim();
      const duration = Math.min(480, Math.max(15, Number(args.duration) || 60));
      if (!title || !dateInput || !timeInput) {
        return {
          content: [{ type: 'text', text: 'Error: title, date, and time are required' }],
          isError: true
        };
      }
      try {
        const dateTime = `${dateInput} ${timeInput}`.trim();
        const escapedDateTime = dateTime.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
        // Parse date/time using system locale (e.g. "tomorrow 2pm", "2024-01-15 14:00")
        const script = `
          tell application "Calendar"
            ...
            set startDate to date "${escapedDateTime}"
...
export const reminderTools: SystemTool[] = [
  ...
  },
  {
    ...
    handler: async (args) => {
      ...
      const listName = String(args.list || 'Reminders').replace(/"/g, '\\"');
      const dueInput = args.dueDate ? String(args.dueDate).replace(/[\r\n]+/g, ' ').trim() : '';
      const escapedDue = dueInput.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
      try {
        const script = dueInput ? `
          tell application "Reminders"
            set theList to list "${listName}"
            set dueDate to date "${escapedDue}"
            make new reminder at end of reminders of theList with properties {name:"${title}", due date:dueDate}
...
export const systemTools: SystemTool[] = [
  ...
  },
  {
    ...
    handler: async (args) => {
      const title = String(args.title || 'Machina').replace(/\\/g, '\\\\').replace(/"/g, '\\"');
