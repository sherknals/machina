...
const _write = (s: string) => process.stdout.write(s);
...
async function startBridge(config: Config, host: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    proc.stdout.on('data', (data) => {
      ...
      if ((str.includes('Machina is online and ready') || str.includes('listening on')) && !started) {
        started = true;
        resolve();
      }
    });
    ...
    proc.on('exit', (code) => {
      if (!started) {
        reject(new Error(`Bridge exited with code ${code ?? 'unknown'}`));
      }
    });
...
async function main() {
  ...
  if (!existsSync(configPath)) {
    log(`${c.red}Error: machina.config.json not found${c.reset}`);
  ...
  try {
    ...
    log(`${c.red}Error: Invalid machina.config.json${c.reset}`);
    process.exit(1);
  }
  ...
  if (isDeployed) {
    log(`  ${c.green}${c.bold}Open Machina:${c.reset} ${c.cyan}${config.deployedUrl}${c.reset}`);
    log('');
    log(`  ${c.dim}API Secret:${c.reset} ${c.white}${config.authToken.slice(0, 32)}${c.reset}`);
  } else if (needsLocalAgent) {
    log(`  ${c.green}${c.bold}Open Machina:${c.reset} ${c.cyan}http://localhost:8787${c.reset}`);
    log('');
    log(`  ${c.dim}API Secret:${c.reset} ${c.white}${config.authToken.slice(0, 32)}${c.reset}`);
  } else {
