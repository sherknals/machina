...
const _centerCol = (textLen: number) => Math.max(1, Math.floor((cols - textLen) / 2));
    ...
    for (const dir of dirs) {
      ...
      try {
        ...
        const validCommands = manifest.commands
          .filter((cmd: RaycastCommand) => cmd && cmd.name && cmd.title)
          .filter((cmd: RaycastCommand) => !cmd.mode || cmd.mode === 'no-view' || cmd.mode === 'view')
          .map((cmd: RaycastCommand) => ({
            name: cmd.name,
            title: cmd.title,
            description: cmd.description || '',
            ...(cmd.arguments && Array.isArray(cmd.arguments) && cmd.arguments.length > 0 && {
              arguments: cmd.arguments.map((arg: RaycastArgument) => ({
                name: arg.name || 'input',
                type: arg.type || 'text',
...
async function _startQuickTunnel(port: number): Promise<string> {
...
async function main() {
  ...
  const modeChoice = await askChoice('How will you use Machina?', [
        ...
        try {
          ...
          await showProgress('Deploying to Cloudflare...', async () => {
            try {
              ...
              const fullErr = err.stdout?.toString() || err.stderr?.toString() || err.message || '';
          ...
          const errMsg = err.message || err.stderr?.toString() || 'Unknown error';
  ...
  if (config.deployed && config.deployedUrl) {
    ...
    moveTo(25, boxX);
    write(c.green + '2. ' + c.white + 'Open Machina:' + c.reset);
    moveTo(26, boxX + 3);
    ...
    moveTo(22, boxX);
    write(c.green + '1. ' + c.white + 'Start Machina:' + c.reset);
    moveTo(23, boxX + 3);
    write(c.green + c.bold + 'npm start' + c.reset);
