...
/**
 * Machina Agent - Remote Mac Control via AI
 *
...
export class SystemAgent extends Agent<Env, SystemState> {
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    if (path.match(/\/schedules\/[^/]+$/) && request.method === "DELETE") {
      ...
      if (scheduleId) {
        try { await this.cancelSchedule(scheduleId); } catch (e) { console.error("[Schedule] Cancel error:", e); }
        const registry = this.state.scheduleRegistry || [];
        this.setState({
          ...this.state,
  ...
  buildSystemPrompt(tools: Tool[]): string {
    ...
    return `You are Machina, a personal AI assistant that controls a Mac remotely. Be helpful and concise.
...
Be brief. Don't explain - just do it.`;
  ...
  parseResponse(content: string): {
    ...
    while ((actionMatch = actionRegex.exec(content)) !== null) {
      ...
      } catch { /* Invalid JSON in action block, skip */ }
    }
    ...
    if (scheduleMatch) {
      ...
      } catch { /* Invalid JSON in schedule block, skip */ }
    }
    ...
    if (prefMatch) {
      ...
      } catch { /* Invalid JSON in preference block, skip */ }
    }
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (missingEnv.length > 0) {
      ...
      return new Response(`Missing required env vars: ${missingEnv.join(", ")}`, { status: 500 });
    }
    ...
    if (url.pathname === "/api" && request.method === "GET") {
      return Response.json({
        name: "Machina",
        version: "1.0.0",
        endpoints: ["/", "/agents/machina-agent/chat", "/agents/machina-agent/schedules"],
      });
    }
    ...
    if (url.pathname.startsWith("/agents/")) {
      ...
      const prefix = "/agents/machina-agent";
